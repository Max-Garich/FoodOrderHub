...
function fileToPhotoDataUrl(file, maxSize = 480, quality = 0.75) {
            ...
            return (
              <div className="card dish-card" key={item.id}>
                <div className="dish-photo-box">
                  {item.photoUrl ? (
                    <img className="dish-photo" src={item.photoUrl} alt={item.name} loading="lazy" />
                  ) : (
...
// Модалка «В меню»: цена и порции предзаполнены последними использованными
    ...
    <div className="modal-overlay modal-center" onClick={onClose}>
      <div className="modal-dialog" onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16 }}>
          {item.photoUrl ? (
            <img className="dish-photo" src={item.photoUrl} alt={item.name} style={{ width: 56, height: 56 }} />
          ) : (
            <div className="dish-photo dish-photo-placeholder" style={{ width: 56, height: 56 }}>🍽️</div>
          )}
